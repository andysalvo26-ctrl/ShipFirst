...
@MainActor
fileprivate func handleViewError(_ error: Error, appState: AppState, errorMessage: inout String?) {
...
struct KodosWelcomeAuthView: View {
    ...
    var body: some View {
        NavigationStack {
            KodosSurface(
                ...
                subtitle: "Set up your app idea and produce a trusted draft plan."
            ) {
                KodosCard {
    ...
    private func authenticate() async {
        ...
        do {
            ...
            handleViewError(error, appState: appState, errorMessage: &errorMessage)
        }
    }
}
        ...
        Dictionary(grouping: runs, by: \ .projectID)
            .map { key, groupedRuns in
                ...
                return ProjectBucket(
                    ...
                    latestRevision: sorted.first?.cycleNo ?? 1,
                        ...
                        NavigationLink {
                            ...
                            HStack(spacing: 12) {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("Latest plan: \(project.latestStatus.capitalized), revision \(project.latestRevision)")
                                        .font(.subheadline)
                                        .foregroundStyle(.secondary)
                                }
                                Spacer()
...
struct KodosProjectRunsView: View {
    ...
    var body: some View {
        KodosSurface(
            ...
            subtitle: "Each plan can have multiple revisions."
        ) {
            KodosCard {
                Button {
    ...
    private func startNewRun() async {
        ...
        do {
            ...
            do {
                ...
                // Keep forward momentum: a successful revision start should still open the plan.
            }
        } catch {
            handleViewError(error, appState: appState, errorMessage: &errorMessage)
        }
...
struct KodosRunHomeView: View {
    ...
    var body: some View {
        KodosSurface(
            title: safeTitle(projectTitle),
            subtitle: "\(stageLabel.title) • Revision \(run.cycleNo)"
        ) {
            KodosCard {
                Text(stateSummary)
                    .font(.body)
...
struct KodosTurnLoopView: View {
    ...
    @State private var planStage: PlanStage?
    @State private var understandingIndicator: NextTurnResult.UnderstandingIndicator?
    @State private var suggestedGenerationMode: GenerationMode = .fastDraft
    @State private var domainKitKey: String?
    @State private var docStrengths: [DocStrength] = []
    ...
    private var stageSummaryText: String? {
        guard let planStage else { return nil }
        switch planStage {
        case .ideaCapture:
            return "Stage: Idea capture"
        case .draftReady:
            return "Stage: Ready for first draft generation"
        case .draftStrengthening:
            return "Stage: Draft strengthening"
        case .builderReady:
            return "Stage: Builder-ready plan"
        }
    }

    private var strengthSummaryText: String? {
        guard !docStrengths.isEmpty else { return nil }
        let average = Int(Double(docStrengths.map(\.score).reduce(0, +)) / Double(docStrengths.count))
        let strongCount = docStrengths.filter { $0.qualityTier == .strong }.count
        return "Quality signal: \(average)/100 avg, \(strongCount) strong role(s), domain kit: \(domainKitKey ?? "generic")."
    }

    private var understandingTitleText: String? {
        understandingIndicator?.title
    }

    private var understandingSummaryText: String? {
        understandingIndicator?.summary
    }

    private var understandingNextStepText: String? {
        understandingIndicator?.nextStep
    }

    var body: some View {
        KodosSurface(
            title: safeTitle(projectTitle),
            subtitle: "\(turnStage.title) • Revision \(activeCycleNo)"
        ) {
            if let readiness {
            ...
            if stageSummaryText != nil || understandingSummaryText != nil || strengthSummaryText != nil {
                KodosCard {
                    if let stageSummaryText {
                        Text(stageSummaryText)
                            .font(.subheadline.weight(.semibold))
                    }
                    if let understandingTitleText {
                        Text(understandingTitleText)
                            .font(.subheadline.weight(.semibold))
                    }
                    if let understandingSummaryText {
                        Text(understandingSummaryText)
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                    if let understandingNextStepText {
                        Text(understandingNextStepText)
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                    if let strengthSummaryText {
                        Text(strengthSummaryText)
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                }
        ...
        .sheet(isPresented: $showCommitReview) {
            KodosCommitReviewView(
                ...
                suggestedMode: suggestedGenerationMode,
    ...
    private func reloadState() async {
        ...
        do {
            ...
            intakeTurns = try await turns
            decisions = try await decisionRows
            if currentPromptText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
               let latestSystemTurn = intakeTurns.last(where: { $0.actorType == .system }) {
            ...
            if planStage == nil {
                planStage = canCommit ? .draftReady : (intakeTurns.isEmpty ? .ideaCapture : .draftStrengthening)
            }
    ...
    private func applyTurnResult(_ result: NextTurnResult) {
        ...
        let effectivePrompt = result.question?.prompt ?? result.assistantMessage
        currentPromptText = userFacingAssistantText(effectivePrompt)
        artifactState = result.artifact
        planStage = result.planStage
        understandingIndicator = result.understandingIndicator
        if let suggested = result.suggestedGenerationMode {
            suggestedGenerationMode = suggested
        }
        domainKitKey = result.domainKitKey
        docStrengths = result.docStrengths ?? []
        ...
        // Trust backend commit readiness (it has full context, including doc-strength heuristics),
        // while still allowing local readiness to unlock commit if backend response is conservative.
        ...
        if result.checkpoint?.requiresResponse == true {
            ...
            let sourceOptions = result.question?.options ?? result.options
            let filtered = filterUserOptions(sourceOptions, canCommit: canCommit)
    ...
    private func submitSelectedOptions(_ selected: [InterviewOption]) async {
        ...
        do {
            ...
            if case .turn(let result) = outcome {
                ...
                // If selection produced no visible state transition, send a tiny evidence bridge so the plan advances.
    ...
    private func blankCanvas() async {
        ...
        do {
            ...
            websiteInput = ""
            artifactState = nil
            understandingIndicator = nil
            committedResult = nil
            hasCompletedContextStep = false
            planStage = .ideaCapture
            suggestedGenerationMode = .fastDraft
            domainKitKey = nil
            docStrengths = []
            await reloadState()
        } catch {
    ...
    private func filterUserOptions(_ options: [InterviewOption], canCommit _: Bool) -> [InterviewOption] {
        guard !options.isEmpty else { return [] }
        // Keep backend-provided readiness options visible so "generate now vs strengthen"
        // decisions are not hidden by stricter local heuristics.
        return options
    }
...
struct KodosCommitReviewView: View {
    ...
    @State private var selectedGenerationMode: GenerationMode
    ...
    init(
        ...
        canCommit: Bool,
        blockers: [String],
        suggestedMode: GenerationMode,
        onCommitted: @escaping (CommitContractResult) -> Void,
        onCommitBlocked: @escaping ([String]) -> Void
    ) {
        ...
        self.onCommitBlocked = onCommitBlocked
        _selectedGenerationMode = State(initialValue: suggestedMode)
    }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Picker("Generation mode", selection: $selectedGenerationMode) {
                        Text("Fast Draft").tag(GenerationMode.fastDraft)
                        Text("Strengthen").tag(GenerationMode.strengthen)
                    }
                    .pickerStyle(.segmented)
                    Text(selectedGenerationMode == .fastDraft
                        ? "Fast Draft produces a strong MVP quickly."
                        : "Strengthen pushes for a richer plan using your existing draft.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
                ...
                Button {
                    ...
                    Text(selectedGenerationMode == .fastDraft ? "Generate Fast Draft" : "Generate Strengthened Draft")
                        .frame(maxWidth: .infinity)
                }
                .buttonStyle(.borderedProminent)
    ...
    private func commit() async {
        ...
        do {
            let outcome = try await api.performTypedAction(
                projectID: projectID,
                cycleNo: cycleNo,
                action: .requestCommit(mode: selectedGenerationMode)
            )
            ...
            if lowerMessage.contains("strengthen mode requires") {
                let strengthenGuidance = "Generate Fast Draft once, then return to Strengthen."
                localBlockers = [strengthenGuidance]
                onCommitBlocked(localBlockers)
                inlineStatusMessage = strengthenGuidance
                return
            }
...
struct KodosDocumentsReviewView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section("Submit") {
                    if isSubmissionLocked {
                        ...
                        Toggle("I reviewed this plan", isOn: $reviewConfirmed)
            ...
            .navigationTitle("Review 10-Doc Plan")
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button("Close") { dismiss() }
                }
